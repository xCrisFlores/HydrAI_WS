...
API_URL = "http://IP:5050/api/consumption"  #Agrega tu IP local
reg = Regression()
X_train, y_train = [], []
...
def send_data_to_api_periodically():
    ...
    while True:
        ...
        with lock:
            print(f"⏳ Intentando enviar datos: time_active={time_active_api}, consumption={consumption_api}")
            if time_active_api > 0 or consumption_api > 0:
                if user_id is not None and sensor_id is not None:
                    ...
                    print("⚠️ No se han recibido user_id o sensor_id, no se enviaron datos.")
    ...
    with connection_lock:
        for ws in active_connections:
            try:
                ...
                print(f"📤 Datos enviados a cliente")
            except Exception as e:
                print(f"❌ Error al enviar datos a cliente: {str(e)}")
                to_remove.append(ws)
...
def websocket(ws):
    ...
    with connection_lock:
        ...
        print(f"🔌 Nuevo cliente conectado (total: {len(active_connections)})")

    if latest_data:
        try:
            ws.send(json.dumps(latest_data))
            print("📤 Datos recientes enviados al nuevo cliente")
        except Exception as e:
        ...
        while True:
            ...
            try:
                ...
                if "ping" in formatted_data:
                    print("📡 Ping recibido del cliente", formatted_data)
                    ws.send(json.dumps({"status": "ok", "message": "Servidor esperando datos del ESP8266", "is_response": True}))
                    continue
                ...
                if "source" in formatted_data and formatted_data["source"] == "react_app":
                    if "time_active" in formatted_data:
                        ...
                        try:
                            ...
                            print(f"📊 Predicción con tiempo {tiempo_react}: {predicted_consumption}")
                            ws.send(json.dumps(response_data))
                        except Exception as e:
                            print(f"⚠️ Error en la predicción: {str(e)}")
                            ws.send(json.dumps({"status": "error", "message": "Error en la predicción"}))
                ...
                if "time_active" in formatted_data and "consumption" in formatted_data and "user_id" in formatted_data and "sensor_id" in formatted_data:
                    ...
                    if len(X_train) >= 2:
                        try:
                            ...
                            print(f"⚠️ Error entrenando el modelo: {str(e)}")
